...
| Local API batch exhaustion | Many candidates multiplied by many accessible groups | Stable candidate and item deduplication, production concurrency of one with a hard ceiling of six, lightweight title-first search, 30-second per-request timeout, sixty-second timeout circuit breaker, bounded exponential page retry, five-second bounded query cache, thirty-second group cache, existing 80-candidate page limit, and stale-batch abort/ignore behavior | Identifier-only misses can still require Zotero full-text search; enhanced mode is the faster option for large batches. |
