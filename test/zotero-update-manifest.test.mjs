...
test("development candidate manifest closes over the current built XPI without changing the published channel", async () => {
  ...
  try {
    ...
    assert.equal(item.update_link, "https://candidate.invalid/paper-library-checker-zotero-0.5.0.xpi");
    assert.deepEqual(await readFile(path.join(root, "updates.json")), publishedBefore);
    for (const name of [
      "paper-library-checker-zotero-0.5.0.xpi",
      "paper-library-checker-extension-0.5.0.zip"
    ]) {
