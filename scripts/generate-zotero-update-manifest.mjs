...
export function createUpdateManifest({ packageJson, manifest, xpiSha256 }) {
  ...
  return {
    addons: {
      [zotero.id]: {
        updates: [{
          version,
          update_link: `https://github.com/he-chun/paper-library-checker/releases/download/v${version}/${xpiName}`,
...
async function main() {
  ...
  const output = process.env.PLC_UPDATES_FILE ? path.resolve(process.env.PLC_UPDATES_FILE) : path.join(root, "updates.json");
  ...
  const result = createUpdateManifest({ packageJson, manifest, xpiSha256: createHash("sha256").update(xpi).digest("hex") });
